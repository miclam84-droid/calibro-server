...
@bp.route("/admin/aggiungi-fenomeni-mancanti")
def admin_aggiungi_fenomeni_mancanti():
    """Aggiunge i fenomeni-trucco mancanti (Leidenfrost, sferificazione...) con numero-bersaglio e schede."""
    from flask import request, jsonify
    import os, psycopg2, json
    if request.args.get("s") != os.environ.get("ADMIN_SECRET", ""):
        return jsonify({"errore": "non autorizzato"}), 403
    ...
    FENOMENI = [
        {"id": "fen-leidenfrost", "nome": "Effetto Leidenfrost", "disciplina": "cucina",
         "target": "~193°C", "grandezza": "temperatura padella", "unita": "°C",
         "scheda": {"cosa": "La goccia d'acqua che 'balla' e resta unita sulla padella invece di evaporare subito.",
                    ...
                    "usare": "È il test del cuoco: goccia che sfrigola e sparisce = padella fredda (sotto 150°C); goccia che balla e resta unita = padella pronta (200°C+) per scottare e per la reazione di Maillard senza attaccare.",
                    "creare": "Usa il test prima di scottare carne o saltare verdure: se la goccia balla, il cibo doratura senza attaccarsi.",
                    "misurare": "~193°C è il punto di Leidenfrost dell'acqua. Sopra, la goccia galleggia; sotto, evapora sfrigolando."}},
        {"id": "fen-sferificazione", "nome": "Sferificazione", "disciplina": "cucina",
         "target": "0.5% alginato · 0.5-1% calcio", "grandezza": "concentrazione alginato", "unita": "%",
         "scheda": {"cosa": "La tecnica che trasforma un liquido in sfere con membrana gelatinosa (caviale finto, ravioli liquidi).",
                    "capire": "L'alginato di sodio (da alghe) reagisce col calcio: al contatto forma una membrana gelatinosa che racchiude il liquido. Diretta (liquido nell'alginato→bagno di calcio) o inversa (liquido col calcio→bagno di alginato).",
                    "usare": "Caviale di succhi, sfere che scoppiano in bocca, ravioli liquidi. Base della cucina modernista.",
                    "creare": "Sferificazione diretta: 0.5% alginato nel liquido, immergi gocce in bagno 0.5-1% cloruro di calcio. Inversa per liquidi acidi o con calcio.",
                    "misurare": "Alginato 0.5%, calcio 0.5-1%. Il pH conta: sotto 3.6 la sferificazione diretta fallisce (serve citrato)."}},
        {"id": "fen-brunoise", "nome": "Brunoise (taglio)", "disciplina": "cucina",
         "target": "2-3 mm cubetti", "grandezza": "lato cubetto", "unita": "mm",
         "scheda": {"cosa": "Il taglio a cubetti minuscoli e regolari (2-3mm) delle verdure.",
                    "capire": "La regolarità del taglio garantisce cottura uniforme: pezzi uguali = stessa velocità di cottura. La superficie/volume alta favorisce rilascio di aromi.",
                    "usare": "Base di soffritti fini, guarnizioni, mirepoix elegante. Cottura uniforme e resa estetica.",
                    "creare": "Prima julienne (bastoncini 2-3mm), poi taglia trasversalmente a cubetti 2-3mm.",
                    "misurare": "2-3 mm di lato. Sotto = fine brunoise (1-2mm), sopra = macedonia."}},
        {"id": "fen-punto-fumo", "nome": "Punto di fumo", "disciplina": "cucina",
         "target": "varia per grasso (EVO ~190°C, arachidi ~230°C)", "grandezza": "temperatura", "unita": "°C",
         "scheda": {"cosa": "La temperatura a cui un grasso inizia a fumare e degradarsi, producendo acroleina (tossica) e sapori bruciati.",
                    "capire": "Ogni grasso ha il suo punto di fumo: oltre, i trigliceridi si degradano, si formano composti sgradevoli e nocivi. Più il grasso è raffinato/saturo, più alto il punto di fumo.",
                    "usare": "Scegli il grasso giusto per la tecnica: frittura (arachidi ~230°C), saltare (EVO ~190°C), a crudo (EVO extra).",
                    "creare": "Per friggere usa oli ad alto punto di fumo (arachidi, girasole alto oleico). Non superare mai il punto di fumo.",
                    "misurare": "EVO ~190°C, arachidi ~230°C, burro ~150°C, burro chiarificato ~250°C."}},
    ]
    try:
        conn = psycopg2.connect(os.environ["DATABASE_URL"]); cur = conn.cursor()
        aggiunti = 0
        for f in FENOMENI:
            cur.execute("SELECT id FROM nodes WHERE id = %s", (f["id"],))
            if cur.fetchone():
                continue  # già esiste
            data = {"nome": f["nome"], "disciplina": f["disciplina"], "target": f["target"],
                    "grandezza": f["grandezza"], "unita": f["unita"], "scheda": f["scheda"],
                    "tipo_fenomeno": "trucco"}
            cur.execute("""INSERT INTO nodes (id, name, type, data) VALUES (%s, %s, 'Fenomeno', %s)""",
                        (f["id"], f["nome"], json.dumps(data)))
            conn.commit(); aggiunti += 1
        cur.close(); conn.close()
        return jsonify({"aggiunti": aggiunti, "fenomeni": [f["nome"] for f in FENOMENI]})
    except Exception as e:
