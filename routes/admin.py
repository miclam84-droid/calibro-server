...
@bp.route("/admin/arricchisci-ricette")
def admin_arricchisci_ricette():
    ...
    import ai_gateway as GW
    disc = request.args.get("disc", "")      # opzionale: una disciplina per volta
    limite = int(request.args.get("limite", "5"))  # quante per chiamata (evita timeout)
    solo_vuote = request.args.get("solo_vuote", "1") == "1"  # solo quelle senza procedimento
    conn = _get_conn()
    try:
        cur = conn.cursor()
        q = "SELECT id,nome,disciplina,descrizione,ingredienti,fenomeni,numeri,punto_critico,procedimento FROM ricette"
        cond = []
        if disc: cond.append("disciplina=%s")
        ...
        for row in rows:
            ...
            # salta quelle che hanno gia il procedimento
            ...
            prompt = (
                ...
                f"PUNTO CRITICO: {pc or ''}\n\n"
                f"Rispondi in italiano SOLO con questo JSON (nessun testo extra):\n"
                f'{{"procedimento": [{{"n":1,"testo":"passaggio specifico e reale","numero_chiave":"il numero critico di questo passo (es. 80-85°C) o null"}}], '
            ...
            try:
                ...
                cur.execute("""UPDATE ricette SET
                    procedimento=%s::jsonb, applicazioni=%s::jsonb,
                    tempo_prep=%s, tempo_cottura=%s, difficolta=%s, porzioni=%s
                    WHERE id=%s""",
            ...
            n += 1
        return jsonify({"ok":True, "arricchite":fatte, "saltate_gia_pronte":saltate, "errori":errori,
                        "nota":"richiama con &limite=5 finche saltate copre tutte"})
    except Exception as e:
        return jsonify({"errore": str(e), "trace": traceback.format_exc()[:400]}), 500
    finally:
        _release_conn(conn)
