...
def motore_gelato_endpoint():
    """MOTORE Gelato Pro: bilanciamento + PAC per la temperatura di vetrina."""
    from flask import request, jsonify
    try:
        d = request.get_json(force=True) or {}
...
def motore_cocktail_endpoint():
    """MOTORE Cocktail Pro: diluizione per tecnica (Dave Arnold)."""
    from flask import request, jsonify
    try:
        d = request.get_json(force=True) or {}
...
def motore_caffe_endpoint():
    """MOTORE Caffè Pro: ratio + estrazione (SCA)."""
    from flask import request, jsonify
    try:
        d = request.get_json(force=True) or {}
