...
def ponti_ingrediente(ingrediente):
    ...
    formaggi, cucina, pasticceria. Usa gli abbinamenti del grafo + classificazione per disciplina."""
    from flask import request, jsonify
    import psycopg2 as _pg
    DISCIPLINE = {
    ...
    REGOLE_NOME = {
        "vino": ["vino", "wine", "champagne", "prosecco", "spumante"],
        "cocktail": ["gin", "rum", "vodka", "whisky", "tequila", "bitter", "vermouth", "liquore", "amaro", "cognac", "brandy", "aperol", "campari"],
        "caffè": ["caff", "coffee", "espresso", "moka"],
        "gelateria": ["gelato", "sorbetto", "panna", "latte"],
        "formaggi": ["formagg", "cheese", "mozzarella", "parmigiano", "pecorino", "gorgonzola", "ricotta", "burrata", "cheddar", "brie"],
        "panificazione": ["farina", "lievito", "pane", "pasta", "grano", "segale", "cereale"],
        "pasticceria": ["cioccolat", "zucchero", "vaniglia", "cacao", "miele", "caramello", "marzapane"],
    }
    CUCINA_HINT = ["pomodoro", "basilico", "aglio", "cipolla", "carne", "pesce", "manzo", "pollo", "verdur", "peperone", "zucchin", "melanzana", "fungo", "carota", "sedano", "prezzemolo", "origano", "rosmarino", "salvia", "timo"]
        ...
        for nome, data, ov in righe:
            ...
            for disc, chiavi in DISCIPLINE.items():
                if any(k in testo for k in chiavi):
                    _aggiungi(disc, nome, ov); trovato = True
            for disc, chiavi in REGOLE_NOME.items():
                if any(k in nl for k in chiavi):
                    _aggiungi(disc, nome, ov); trovato = True
            if any(k in nl for k in FRUTTA):
                _aggiungi("cucina", nome, ov); _aggiungi("pasticceria", nome, ov); _aggiungi("cocktail", nome, ov); trovato = True
            if any(k in nl for k in CUCINA_HINT):
...
def lista_principi():
    ...
    try:
        ...
        for rid, nome, data in righe:
            dd = data if isinstance(data, dict) else {}
            scheda = dd.get("scheda", "")
            desc = ""; formula = ""
            if isinstance(scheda, dict):
                desc = (scheda.get("descrizione") or scheda.get("spiegazione") or scheda.get("testo") or
                        scheda.get("sintesi") or scheda.get("intro") or scheda.get("cosa") or
                        scheda.get("enunciato") or "")
                formula = scheda.get("formula", "") or scheda.get("equazione", "")
                if not desc:
                    for v in scheda.values():
                        if isinstance(v, str) and len(v) > 30:
                            desc = v; break
            elif isinstance(scheda, str):
                desc = scheda
            principi.append({
                "id": rid, "nome": nome,
                "descrizione": str(desc)[:250], "formula": str(formula)[:60],
            })
        _cur.close(); _release_conn(_c)
...
def flavour_network(ingrediente):
    """Grafo Sapori - MOTORE FUSO (logica Composer): abbinamenti per composti condivisi (analogia,
    esclusi i parenti) + proprieta sensoriali complementari (contrasto). Il perche sempre dove ci sono dati."""
    from flask import request, jsonify
    ...
    def _radice(nome):
        ...
        n = nome.lower().strip()
        for p in ['pomodoro','limone','arancia','farina','cioccolato','aceto','olio','manzo','vino',
                  'formaggio','pepe','sale','zucchero','miele','riso','pane','mela','uva','fragola',
                  'basilico','patata','carciofo','melanzana','zucchina','peperone','radicchio','fagiol']:
            if p in n: return p
    ...
    try:
        ...
        nodi = []; visti = set([radice_centro])

        # ANALOGIA: composti condivisi, ESCLUSI i parenti (stessa radice)
        if len(comp_centro) >= SOGLIA:
            _cur.execute("""SELECT n.name, COUNT(*) ov FROM edges e JOIN nodes n ON n.id=e.from_id
                            ...
                            GROUP BY n.name HAVING COUNT(*) >= %s ORDER BY ov DESC LIMIT 60""",
                         (list(comp_centro), id_centro, SOGLIA))
            for nome, ov in _cur.fetchall():
                rad = _radice(nome)
                if rad in visti: continue  # ESCLUDE parenti e gia visti
                visti.add(rad)
                ...
                cond = comp_centro & set(r[0] for r in _cur.fetchall())
                if len(cond) < SOGLIA: continue
                forza = min(99, int(len(cond)/max(len(comp_centro),1)*100))
                nomi_c = [x.replace('ahn_comp_','').replace('_',' ') for x in list(cond)[:5]]
                nodi.append({"nome": nome, "forza": max(forza,35), "tipo":"analogia",
                             "perche": {"composti_condivisi": nomi_c, "n_condivisi": len(cond)}})
                if len(nodi) >= n_max: break

        # CONTRASTO: proprieta complementari (come il Composer). Aggiungo SEMPRE qualche contrasto sensato.
        if prop_centro:
            ...
            regole=[]
            if g>=6: regole.append(('acido',6,'taglia il grasso'))
            if a>=6: regole.append(('grasso',6,'ammorbidisce l acido'))
            if d2>=7: regole.append(('amaro',5,'bilancia il dolce'))
            if u>=6: regole.append(('acido',5,'ravviva l umami'))
            if pi>=6: regole.append(('dolce',6,'calma il piccante'))
            if am>=6: regole.append(('grasso',6,'addolcisce l amaro'))
            for prop_t, soglia_t, motivo in regole[:2]:
                ...
                for nome, data in _cur.fetchall():
                    rad=_radice(nome)
                    if rad in visti: continue
                    ddn = data if isinstance(data, dict) else _j.loads(data)
                    pv = ddn.get('proprieta',{}).get(prop_t,0)
                    if pv >= soglia_t: cand.append((nome, rad, pv))
                cand.sort(key=lambda x:-x[2])
                for nome, rad, pv in cand[:4]:
                    if rad in visti or len(nodi)>=n_max: continue
                    visti.add(rad)
                    nodi.append({"nome": nome, "forza": 45+pv*3, "tipo":"contrasto",
                                 "perche_sensoriale": motivo})
...
def assistente_galileo():
    """ASSISTENTE GALILEO: chat generalista inclusa nell'abbonamento. Modello economico, cap ENERGIA
    per account, degrado gentile, escalation al dominio. Solo testo."""
    from flask import request, jsonify
