...
window._moduli = window._moduli || { chat:false, lezioni:false, flavour:false, dna:false, menu:false, ricette:false, motori:false, planner:false, composer:false, grafo:false, scienza:false, biodiversita:false, laboratorio:false, galileo:false, menubuilder:false, possibilita:false };
function _caricaModulo(nome){
  return new Promise(function(resolve){
    if(window._moduli[nome]){ resolve(); return; }
    var s=document.createElement('script');
    s.src='/static/matter-'+nome+'.js?v='+(window._MB_VER||'1');
    s.onload=function(){ window._moduli[nome]=true; resolve(); };
    s.onerror=function(){ resolve(); }; // non blocco la UI se fallisce
...
function switchTab(t){
  ...
  if(t==='quaderno' && !window._moduli.ricette){ _caricaModulo('ricette'); }
...
window._chatConContesto = function(tipo, oggetto){
  ...
  else if(tipo==='planner'){ chip=[['Abbassa i costi','Come rendo questo menu più economico da produrre senza perdere qualità? Guarda i piatti che ho scelto.'],['Più varietà','Come aumento la varietà del menu evitando ripetizioni?'],['Bilancia le proteine','Il menu è bilanciato tra carne, pesce e vegetariano? Come miglioro?']]; }
...
async function _caricaImmagineRicetta(id){
  try{
    ...
    if(j.tipo==='foto' && j.url){
      // l'autore dal backend è già formattato ("Foto: Autore (licenza) / fonte")
      var attr = j.autore || j.fonte || '';
      box.innerHTML='<img src="'+_escV(j.url)+'" alt="" loading="lazy">'
        + (attr ? '<div class="rg-img-attr">'+_escV(attr)+'</div>' : '');
...
async function _caricaPostiFounding(){
  ...
  if('serviceWorker' in navigator){
    window.addEventListener('load', ()=>{
      ...
      setTimeout(function(){ if(typeof _caricaModulo==='function'){ _caricaModulo('chat'); _caricaModulo('lezioni'); _caricaModulo('flavour'); _caricaModulo('dna'); _caricaModulo('menu'); _caricaModulo('ricette'); } }, 1200);
    });
  }
